...
try {
  ...
  if (fs.existsSync(packageJsonPath)) {
    ...
    if (packageJson.scripts) {
      packageJson.scripts['vercel-build'] = 'node fix-vercel-build.mjs && ROLLUP_SKIP_NATIVE=true DISABLE_BINARY=true vite build';
...
if (!buildResult.success) {
  ...
  // Criar arquivo window.env.js no diretório dist
  const windowEnvContent = `// Generated during build
window.env = {
  DEPLOYMENT: 'vercel',
  APP_NAME: 'Escola Digital 3D',
  HOME_PATH: 'client/src/pages/Home.tsx',
  DEFAULT_ROUTE: '/'
};`;

  fs.writeFileSync(path.join(distDir, 'window.env.js'), windowEnvContent);
  console.log('✅ Arquivo window.env.js criado no diretório dist');
...
<body>
  ...
  if (fs.existsSync(clientRedirectsFile)) {
    ...
    // Criar _redirects no dist se não existir no client/public
    const redirectsContent = `# Redirect Sistema de Gestão Escolar to Home page
/Sistema* / 301!
...
/* /index.html 200`;
  ...
  // Criar um arquivo vercel.json no diretório raiz se não existir
