...
import { 
  ...
  CheckSquare, User, ClipboardList, BarChart, Clock, TrendingUp, Filter, ListChecks, 
  PlusCircle, X, CheckCircle, Edit, Upload, Eye, RefreshCw, Edit2, FilePlus, Check
} from "lucide-react";
  ...
  return (
    <div className="min-h-screen flex bg-gradient-to-br from-indigo-50 to-blue-50">
      ...
      <main className="flex-1 p-8 max-w-screen-2xl mx-auto">
                ...
                <table className="w-full text-left">
                  ...
                  <tbody>
                    {mockAlunos[turmaSelecionada as keyof typeof mockAlunos].map((aluno) => (
                      <tr key={aluno.id} 
                        className={`border-b last:border-b-0 hover:bg-blue-50 ${alunoSelecionado?.id === aluno.id ? 'bg-blue-50' : ''}`}
                    ...
                    {atividadeSelecionada && (
                      <div className="mt-2">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full inline-flex items-center gap-1 ${
                          ...
                          {atividadeSelecionada.status === 'Publicado' && <Check className="w-3 h-3 inline mr-1" />}
          ...
          >
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="bg-green-50 rounded-lg p-3 flex items-center justify-between">
                        <span className="text-sm text-green-700 flex items-center gap-1">
                          <BarChart className="w-4 h-4" /> Média
                        </span>
              ...
              <div className="md:col-span-3 bg-white rounded-xl shadow-md overflow-hidden">
                <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-indigo-50 to-white">
                  <div className="flex flex-wrap justify-between items-center gap-4">
                    <div>
                      <h3 className="text-lg font-medium text-gray-900 flex items-center gap-2">
                        <ClipboardList className="w-5 h-5 text-indigo-600" /> Notas - Turma {turmaSelecionada}
                      </h3>
                      <p className="text-sm text-gray-500 mt-1">Lançamento de notas para {mockAlunos[turmaSelecionada as keyof typeof mockAlunos].length} alunos</p>
                    </div>
                    ...
                    <div className="flex gap-2 items-center">
                      ...
                      >
                        <FileText className="w-4 h-4 mr-1" /> Exportar
                      </Button>
                      <Button
                        className="bg-indigo-600 hover:bg-indigo-700"
                        onClick={() => toast.success("Notas salvas com sucesso!")}
                      >
                        <CheckSquare className="w-4 h-4 mr-1" /> Salvar
                      </Button>
                    </div>
                  </div>
                </div>
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="text-indigo-700 border-b bg-indigo-50/50">
                        ...
                        <th className="py-3 px-4 font-medium text-center">Prova Bimestral</th>
                        <th className="py-3 px-4 font-medium text-center">Trabalho em Grupo</th>
                        <th className="py-3 px-4 font-medium text-center">Atividades em Sala</th>
                        <th className="py-3 px-4 font-medium text-center">Participação</th>
                        <th className="py-3 px-4 font-medium text-center">Média Final</th>
                        <th className="py-3 px-4 font-medium text-center">Ações</th>
                      </tr>
                    </thead>
                    <tbody>
                      {mockAlunos[turmaSelecionada as keyof typeof mockAlunos].map((aluno, index) => {
                        ...
                        return (
                          ...
                          >
                            <td className="py-4 px-4">
                              <div className="font-medium text-indigo-900">{aluno.nome}</div>
                            ...
                            <td className="py-4 px-4">
                              <div className="flex justify-center">
                                <input 
                                  ...
                                  max="10"
                                  step="0.1"
                                  className="w-16 border border-gray-300 rounded-lg p-2 text-center focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all hover:border-indigo-300"
                                  defaultValue={notaProva}
                                  onChange={(e) => {
                                    const value = parseFloat(e.target.value);
                                    if (value < 5) e.target.classList.add('bg-red-50', 'text-red-700');
                                    else if (value < 7) e.target.classList.add('bg-amber-50', 'text-amber-700');
                                    else e.target.classList.add('bg-green-50', 'text-green-700');
                                  }}
                                />
                              </div>
                            </td>
                            ...
                            <td className="py-4 px-4 text-center">
                              <div className="flex gap-1 justify-center">
                                <button className="p-1.5 text-indigo-600 hover:bg-indigo-100 rounded-md transition-colors" title="Comentários">
                                  <MessageCircle className="w-4 h-4" />
                                </button>
                                <button className="p-1.5 text-green-600 hover:bg-green-100 rounded-md transition-colors" title="Histórico de notas">
                                  <BarChart className="w-4 h-4" />
                                </button>
                                <button className="p-1.5 text-red-600 hover:bg-red-100 rounded-md transition-colors" title="Limpar notas">
                                  <X className="w-4 h-4" />
                                </button>
                              </div>
                            </td>
                ...
                <div className="p-4 border-t border-gray-200 bg-gray-50">
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    <div className="flex gap-6 items-center">
                      ...
                      <div className="flex gap-2">
                        <Button variant="outline" size="sm" onClick={() => toast.success("Recuperação agendada!")}>
                          Agendar Recuperação
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => toast.success("Notificações enviadas aos pais!")}>
                          Notificar Pais
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => toast.success("Notas zeradas!")}>
                          Zerar Notas
                        </Button>
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white rounded-xl shadow-md p-6">
                <h3 className="text-lg font-medium text-gray-900 mb-4 flex items-center gap-2">
                  <BarChart className="w-5 h-5 text-indigo-600" /> Distribuição de Notas
                </h3>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={[
              ...
              <div className="bg-white rounded-xl shadow-md p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-blue-50 rounded-lg p-4 hover:shadow-md transition-shadow">
                    <h4 className="text-sm font-medium text-blue-700 mb-1">Média da Turma</h4>
                    <p className="text-2xl font-bold text-blue-700">{(mockAlunos[turmaSelecionada as keyof typeof mockAlunos].reduce((acc, curr) => acc + curr.media, 0) / 
                  ...
                  <div className="bg-amber-50 rounded-lg p-4 hover:shadow-md transition-shadow">
                ...
                <div className="mt-6">
                  <h4 className="text-sm font-medium text-gray-700 mb-2 flex items-center justify-between">
                    <span>Observações para o Boletim</span>
                    <span className="text-xs text-indigo-600">Visível para coordenação e pais</span>
                  </h4>
                  <textarea 
        ...
        {section === 'horarios' && (
          <div>
            <h1 className="text-2xl font-bold text-gray-800 mb-6">Meus Horários</h1>
            
            <div className="bg-white rounded-xl shadow p-6 mb-6">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-medium text-gray-900">Quadro de Horários Semanal</h3>
                <div className="flex gap-2">
                  <Button 
                    variant="outline"
